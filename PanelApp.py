...
class PanelAPP_API():
    ...
    def __init__(self):
        ...
        self.list_of_genes = "https://bioinfo.extge.co.uk/crowdsourcing/WebServices/search_genes/all/?format=json&panel_name="
    ...
    def get_list_of_panels(self):
        ...
        for panel in json_results["result"]:
            ...
            toople = (str(panel["Name"]), float(panel["CurrentVersion"]))
    ...
    def get_genes_in_panel(self):
        ...
        for i in self.dict_of_panels:
            ...
            # the response package retrieves the results of the url search
            response = requests.get(self.list_of_genes + panelname)
            ...
            for gene in json_results["results"]:
                ensemblid_list=[]
                for id in gene["EnsembleGeneIds"]:
                    if len(id)>2:
                        ensemblid_list.append(str(id))
                
                # some genes have multiple ensembl gene ids. combine these into a sql friendly string
                ensemblid = "'" + '\',\''.join(ensemblid_list) + "'"  # plan is to use if gene in this strin
        ...
        for i in self.dict_of_panels:
            for j in self.dict_of_panels[i]:
                if len(self.dict_of_panels[i][j])==0:
                    ...
                    outputfile.write(str(i[0])+"_"+str(i[1])+"_"+j+":"+str(self.dict_of_panels[i][j])+"\n")
        outputfile.close()
